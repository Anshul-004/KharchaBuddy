  ...
  const Index = () => {
    ...
    const openCamera = async () => {
      ...
      const result = await ImagePicker.launchCameraAsync({
        // allows to get full image resolution, no cropping
        allowsEditing: false,
        quality: 1,
      });
  
      if (!result.canceled) {
        console.log('Invoice Image URI:', result.assets[0].uri);
      ...
      <SafeAreaView className="flex-1 bg-white">
        <ScrollView className="flex-1">
          ...
          <View className="flex-row items-center justify-between bg-white shadow-sm">
            ...
            <Image 
              source={images.logo}
              className="w-24 h-24 rounded-2xl ml-3"
            />
          ...
          <View>
              ...
              <Text className="text-gray-900 text-2xl font-semibold">John Doe</Text>
            </View>
            ...
            <View className="flex-row justify-between mt-6">
              ...
              <Text className="text-red-500 text-xl font-semibold">$ 1830.00</Text>
            </View>
          </View>

          {/* Recent Transactions, map the scanned invoice data here, for now dummy data is placed */}
          <View className="px-4 mt-6 mb-10">
            <Text className="text-gray-900 text-xl font-semibold mb-4">Recent Transactions</Text>
        ...
        <View className="absolute right-5 bottom-32">
          {/* Scan Invoice Button */}
          <Animated.View style={{ transform: [{ translateY: scanInvoiceTranslateY }] }} className="absolute right-2 bottom-0">
            <TouchableOpacity 
          ...
          {/* Add Manually Button */}
          <Animated.View style={{ transform: [{ translateY: addManuallyTranslateY }] }} className="absolute right-2 bottom-0">
            <TouchableOpacity 
